...
const matchAlgorithm ={
    senior: async (req,res) => {
        ...
        const matches = [];
            ...
            volunteers.forEach(volunteer => {
                ...
                if (senior.interests.some(interest => volunteer.skills.includes(interest)) && senior.city === volunteer.city) {
                    matches.push({name:volunteer.name,email:volunteer.email,skills:volunteer.skills});
                }
            });
        ...
        return res.status(401).json({status:"Not Ok",message:"Match not found", matches:matches})
    },
        ...
        const matches = [];
            ...
            seniors.forEach(senior => {
                ...
                if (volunteer.skills.some(skill => senior.interests.includes(skill)) && volunteer.city === senior.city) {
                    matches.push({name:senior.name,email:senior.email,interests:senior.interests});
                }
            });
