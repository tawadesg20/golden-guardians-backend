...
const loginVerifyMiddleware = {
    senior:{
        isAllDetails:(req,res,next)=>{
            ...
            if(!loginotp) errors.loginotp="login otp is required";
        ...
        isNotExists:async (req,res,next)=>{
            const {email} = req.body;
            const userAlreadyVerified = await seniorModel.findOne({email:email,loginverified:true})
        ...
        isCorrectOtp:async (req,res,next)=>{
            const {email,loginotp} = req.body;
            const correctOtp = await seniorModel.findOne({email:email,loginotp:loginotp})
            if(!correctOtp)
    ...
    volunteer:{
        isAllDetails:(req,res,next)=>{
            ...
            if(!email) errors.email="email number is required";
            if(!loginotp) errors.loginotp="loginotp is required";
        ...
        isNotExists:async (req,res,next)=>{
            const {email} = req.body;
            const userAlreadyVerified = await volunteerModel.findOne({email:email,loginverified:true})
            if(userAlreadyVerified)
                return res.status(401).json({status:false,message:"user already verified"})
        ...
        isCorrectOtp:async (req,res,next)=>{
            const {email,loginotp} = req.body;
            const correctOtp = await volunteerModel.findOne({email:email,loginotp:loginotp})
            if(!correctOtp)
                return res.status(401).json({status:false,message:"incorrect otp, submit again"})
            next()
        }
